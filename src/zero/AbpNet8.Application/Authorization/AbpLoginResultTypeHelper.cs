...
namespace AbpNet8.Authorization
{
    public class AbpLoginResultTypeHelper : AbpServiceBase, ITransientDependency
    {
        ...
        public string CreateLocalizedMessageForFailedLoginAttempt(AbpLoginResultType result, string usernameOrEmailAddress, string tenancyName, DateTime? locked)
        {
            switch (result)
            {
                case AbpLoginResultType.Success:
                ...
                case AbpLoginResultType.LockedOut:
                    return locked != null ? L("UserLockedOutMessage{0}", locked?.ToString("dd/MM/yyyy HH:mm:ss")) : L("UserLockedOutMessage");
                default: //Can not fall to default actually. But other result types can be added in the future and we may forget to handle it
                    Logger.Warn("Unhandled login fail reason: " + result);
