...
namespace AbpNet8.EntityFrameworkCore.Seed.Host
{
    public class DefaultLanguagesCreator
    {
        public static List<ApplicationLanguage> InitialLanguages => GetInitialLanguages();

        private readonly AbpNet8DbContext _context;

        private static List<ApplicationLanguage> GetInitialLanguages()
        {
            var tenantId = AbpNet8Consts.MultiTenancyEnabled ? null : (int?)MultiTenancyConsts.DefaultTenantId;
        ...
        public DefaultLanguagesCreator(AbpNet8DbContext context)
        {
            _context = context;
        }
