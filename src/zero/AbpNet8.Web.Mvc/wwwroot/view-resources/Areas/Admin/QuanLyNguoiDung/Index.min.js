(function () {
    $(function () {
        ...
        var _dmnguoidungService = abp.services.qlhd.quanLyNguoiDung;

        var _doanhNghiep_GoiDichVuService = abp.services.qlhd.doanhNghiep_GoiDichVu;
        ...
        var customDataSource = new DevExpress.data.CustomStore({
            ...
            load: function (loadOptions) {
                ...
                parmaforall = params;
                $.getJSON(abp.appPath + "api/services/qlhd/QuanLyNguoiDung/GetAllItem", params)
                    ...
                    cellTemplate: function (container, options) {
                        ...
                        if (options.data.nguoiDung_TrangThai == true) {
                            if (abp.auth.isGranted('Admin.HeThong.NguoiDung.DoiMatKhau')) {
                                txt += `<a class="btn btn-sm btn-icon btn-icon-md changepass" title="Đổi mật khẩu" style="margin-top: -10px;margin-left: 10px;color:#E1B74B;" href="#" data-id="${options.data.userId}" data-tenant="${options.data.tenantId}"><i class="flaticon-refresh icon-color" style="font-size: 1.4rem;"></i></a>`;
                            }
                            if (abp.auth.isGranted('Admin.HeThong.NguoiDung.Update')) {
                                txt += `<a class="btn btn-sm btn-icon btn-icon-md edit" title="Chỉnh sửa" style="margin-top: -10px;color:#169BD5;margin-left: 15px;" href="#" data-id="${options.data.userId}" data-tenant="${options.data.tenantId}"><i class="fa fa-edit icon-color"></i></a>`;
                            }
                            if (abp.auth.isGranted('Admin.HeThong.NguoiDung.Khoa')) {
                                txt += `<a class="btn btn-sm btn-icon btn-icon-md lock" title="Khóa" style="margin-top: -10px;margin-left: 10px;color:red;" data-id="${options.data.userId}" data-tenant="${options.data.tenantId}" href="#"><i class="fa fa-lock icon-color"></i></a>`;
                            }
                            if (abp.auth.isGranted('Admin.HeThong.NguoiDung.Delete')) {
                                txt += `<a class="btn btn-sm btn-icon btn-icon-md delete" title="Xóa" style="margin-top:-10px;margin-left: 10px;color:red;" data-id="${options.data.id}" data-tenant="${options.data.tenantId}" href="#"><i class="fa fa-trash icon-color"></i></a>`;
                            }
                        } else {
                            if (abp.auth.isGranted('Admin.HeThong.NguoiDung.DoiMatKhau')) {
                                txt += `<a class="btn btn-sm btn-icon btn-icon-md changepass" title="Đổi mật khẩu" style="margin-top: -10px;margin-left: 10px;color:#E1B74B;" href="#" data-id="${options.data.userId}" data-tenant="${options.data.tenantId}"><i class="flaticon-refresh icon-color" style="font-size: 1.4rem;"></i></a>`;
                            ...
                            if (abp.auth.isGranted('Admin.HeThong.NguoiDung.Update')) {
                                txt += `<a class="btn btn-sm btn-icon btn-icon-md edit" title="Chỉnh sửa" style="margin-top: -10px;margin-left: 15px;color:#169BD5;" href="#" data-id="${options.data.userId}" data-tenant="${options.data.tenantId}"><i class="fa fa-edit icon-color"></i></a>`;
                            }
                            if (abp.auth.isGranted('Admin.HeThong.NguoiDung.Khoa')) {
                                txt += `<a class="btn btn-sm btn-icon btn-icon-md unlock" title="Mở khóa" style="margin-top: -10px;margin-left: 10px;color:#169BD5;" data-id="${options.data.userId}" data-tenant="${options.data.tenantId}" href="#"><i class="fa fa-lock-open icon-color"></i></a>`;
                            }
                            if (abp.auth.isGranted('Admin.HeThong.NguoiDung.Delete')) {
                                txt += `<a class="btn btn-sm btn-icon btn-icon-md delete" title="Xóa" style="margin-top:-10px;margin-left: 10px;color:red;"" data-id="${options.data.id}" data-tenant="${options.data.tenantId}" href="#"><i class="fa fa-trash icon-color"></i></a>`;
                            }
        ...
        function canhbao() {
            _doanhNghiep_GoiDichVuService.canhBaoGoiDichVuTongHop().done(function (res) {
                if (res.success == false) {
                    abp.notify.warn(res.message);
                }
            });
        }
